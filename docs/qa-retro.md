...
## 2026-09-07: Cascade voice local trial must retain natural pauses

Issue #263's real PCM microphone fixture contains a normal 500 ms pause. A 450 ms endpoint submitted its first clause, then the second clause's onset aborted ASR and silently lost that first clause. Keep unaccepted audio and merge it into the replacement utterance; clear it only after transcript acceptance, failure, or mic close. The regression uses the same paused sentence rather than increasing the silence threshold until one fixture passes.

The same real provider trial exposed two existing integration assumptions: SiliconFlow rejects a system message inserted after user messages when research rejoins persona context, and quiet CosyVoice speech (peak 50 ms RMS around 0.07) never crosses the playback mouth driver's 0.08 noise gate. Keep the final research policy in the leading system message, preserve research isolation, and use a speech-output-specific mouth gate. Actual AudioBuffer starts/stops, model pixels, mouth motion, complete transcription, and read-page evidence are separate acceptance facts. Generated text history still does not prove the user heard every word.


## 2026-09-16: Proactive Vision needs its own cancellable lifetime

Fork issue #1 reproduced seven missing-abort failures at baseline `3951555`: proactive screen Vision bypassed the chat runtime AbortController, while Stop, new input, configuration replacement, and shutdown only affected other owners. Screen off cleared captured frames but left the provider request alive. A shared in-flight boolean and an unguarded await-to-publish boundary could also allow late output or old cleanup to interfere with a replacement run.

Keep this ephemeral run's signal, cancellation race, and generation in core-runtime. Invalidate before awaiting capture/input work; propagate a real signal to the provider; release the slot without waiting for an uncooperative iterator's `next` or `return`; clear only the matching controller; publish synchronously under the current generation. Do not add images or proactive remarks to session/memory/export. Preserve the existing capture policy and Stop quiet interval.

The ordinary Controls path exposed a second miss: Stop was disabled while only proactive Vision was pending. Existing screen-awareness mode now makes that existing button available (no extra control or UI layout). The focused local SSE gate separately proves unfinished HTTP response closure for Off, new text, and Stop; fresh re-enable; normal user Vision/chat; raw-image non-persistence; visible window bounds and non-fallback model pixels. Core tests separately prove late chunks/rejections and non-cooperative cleanup cannot retain or clear a replacement slot. A connection close alone does not prove stale-result suppression, and a hidden stale result alone does not prove provider abort.


### 2026-09-17 review follow-up: cancellation is not a cooldown reset

Coordinator review of fork PR #2 found that clearing the shared attempt/publication timestamp on every input or config save let an already delivered proactive remark bypass its five-minute quiet interval. Keep attempt throttling separate from successful-publication cooldown: cancellation permits retry of interrupted work but ordinary input and unrelated config saves retain the successful remark's quiet interval. Explicit screen off/on retains this issue's fresh-mode contract. Fake-Date regressions must exercise both immediate suppression and expiry, not just abort signals.

The same review exposed a real admission gap: cancelling at input ingress alone does not stop a new tick while main awaits neko stop or fresh observation context, nor while RuntimeService creates a runtime. Reserve input priority synchronously across those awaits and release in finally. Core-runtime owns identity-safe, idempotent overlapping reservations; config/off cancellation must not release another input's hold. A deferred-preparation regression reproduced publication in this gap before the fix, and core/service tests cover text/audio-end preparation, creation rejection, non-cooperative cancellation, and fresh retry. Keep actual HTTP closure, stale-result suppression, and successful-publication cooldown as distinct acceptance assertions.


## 2026-09-17: Settings vision diagnostics are not chat tests or visual-understanding evidence

Fork issue #3 exposed a missing ordinary Settings path: Test LLM only selected Chat even though Vision/Multimodal slots existed. A successful chat token cannot validate image routing. The new diagnostic sends one deterministic, non-private PNG directly to Vision (or Multimodal only when Vision is empty), never captures the desktop and never creates a runtime/session/memory turn. Fake mode must report that no remote test occurred; a reply proves connectivity only, not image understanding. Return allowlisted result codes, never upstream text, response bodies, keys or stacks.

Generation invalidation alone does not close an HTTP transport. Core-runtime owns independent admission, timeout, abort and stale-result suppression; abort after the first valid token and do not wait indefinitely for a non-cooperative iterator cleanup. Review also found an admission gap while main awaited queued Settings saves: an older click could occupy the slot before the newer click. Reserve the diagnostic before that await, cancel the reservation at relevant edit ingress, and distinguish that edit from the persisted commit the fresh click is waiting for. Deferred-save red/green tests cover this separately from real SSE closure. Shutdown cancels both pending preparation and transport.

The whole Provider section copy audit found stale claims that long-term memory was falling back to basic extraction, that TTS routing was reserved, and that only Vision (not Multimodal fallback) could supply images. Correct both locales and the test that protected the old wording, without enabling paused features. The focused Electron gate clicks the visible task-model shortcut from Controls → Settings with basic chat fields still visible, captures both locales, checks real display/window bounds and non-empty model pixels, and instruments desktop capture to prove zero calls. Keep these facts separate from unit-only lifecycle and safe-error checks.


Coordinator review of fork PR #4 reproduced a renderer-lifetime gap: a per-bridge numeric request counter resets on reload, but WebContents and the core diagnostic remain alive. A reloaded request could either accept an old request's success (ID 1 collision) or remain testing forever (old request 2, new request 1, silent admission rejection). Request IDs must not be reused across documents; competing live requesters need an explicit safe, retryable terminal result. Main only maps full-document navigation, renderer loss and WebContents destruction to a requester signal; core owns identity-safe cancellation of queued-save preparation and transport, and removes requester listeners on settlement. Same-document navigation must not retire a live request. Formal red/green tests cover both old request numbers, saved-config retry and a retired requester's late abort. The focused Electron gate now reloads the same WebContents twice while pending, checks transport closure and no stale success, retries successfully, verifies listener cleanup, exercises real IPC competition, and destroys a separate requester before retrying from Settings. Double-click suppression alone is not renderer reload acceptance.

## 2026-09-17: Settings snapshots must commit only after persistence succeeds

Fork issue #6 reproduced a SettingsController commit-order miss: assigning `this.config` before awaiting `save` exposed pending/failed task-model edits through `getCurrent`, then let a queued unrelated UI edit persist and emit those failed values. Build the candidate from the last committed snapshot and replace the snapshot only after `save` resolves; preserve the existing serial queue, paired legacy/task-model normalization, failure propagation and retry recovery. A notification error after a successful write must not roll back that disk commit or its in-memory snapshot.

Deferred-save red/green tests cover pending reads, first/middle queue failure, unrelated patches, paired fields and waiters through retry. Real temporary-file tests force a filesystem write failure by replacing the parent directory with a file, restore it and verify unchanged bytes/reload, unrelated save and explicit retry; a separate real-write test covers notification failure. This fixes committed state, not the renderer draft: the existing IPC error handler only warns, so no new error toast or automatic input rollback is claimed. Crash-safe/partial-write persistence and notification reconciliation remain separate contracts.
