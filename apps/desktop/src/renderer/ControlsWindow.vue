...
const canStop = computed(() => props.state.screenAwareness.enabled || props.state.cascadeVoice.active || chatStatus.value.canStop || props.state.voiceInput.status === "listening" || props.state.voiceInput.status === "transcribing" || ["starting", "connecting", "ready"].includes(props.state.nekoPlugin.status));
